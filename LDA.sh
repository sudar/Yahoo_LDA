...
echo ${mapred_task_partition}":"`hostname -i` | hadoop dfs -put - ${hostnames_dir}/client-${mapred_task_partition};

wait_for_all 60 ${synch_dir}"/clid_names";

#Parse the client ids & their corresponding hostnames into
#clientids & hostnames array
clid_names=`hadoop dfs -cat ${hostnames_dir}/* | tr "\n" " "`;
...
if [ $maxmem ]; then
  hadoop dfs -copyToLocal ${trained_data}/global/lda.dict.dump training.dict.dump;
  hadoop dfs -copyToLocal ${trained_data}/global/topic_counts topic_counts;
...
if [ ${trained_data} ]; then
  hadoop dfs -copyToLocal ${trained_data}/global/lda.dict.dump training.dict.dump;
  $JAVA Tokenizer | $LDALIBS/formatter --dumpfile=training.dict.dump;
  rm training.dict.dump;
  hadoop dfs -copyToLocal ${trained_data}/global/topic_counts topic_counts;
  dumps=`ls -alh topic_counts | wc -l`;
  dumps=`expr $dumps - 4`;
  ...
  hadoop dfs -copyToLocal "${MY_INP_DIR}" out;
  cd out;
fi;
...
then
  ...
  do 
    hadoop dfs -copyToLocal ${mapred_output_dir}/$i/lda.dict.dump lda.dict.dump.$i;
    i=`expr $i + 1`;
  done;

  $LDALIBS/Merge_Dictionaries --dictionaries=${mapred_map_tasks} --dumpprefix=lda.dict.dump

  hadoop dfs -put lda.dict.dump ${mapred_output_dir}/global/lda.dict.dump;
  check_exit_status $? "dput lda.dict.dump";

  cp -L Merge_Dictionaries.WARNING mergedictionaries.log;
  check_exit_status $? "cp -L WARNING";

  hadoop dfs -put mergedictionaries.log ${mapred_output_dir}/global/mergedictionaries.log;
...
hadoop dfs -get ${mapred_output_dir}/global/lda.dict.dump lda.dict.dump.global
...
hadoop dfs -put lda.ttc.dump ${mapred_output_dir}/global/topic_counts/lda.ttc.dump.${mapred_task_partition}
check_exit_status $? "dput lda.ttc.dump";

cp -L Merge_Topic_Counts.WARNING mergetopics.${mapred_task_partition}.log;
check_exit_status $? "cp -L WARNING";

hadoop dfs -put mergetopics.${mapred_task_partition}.log ${mapred_output_dir}/global/mergetopics.${mapred_task_partition}.log;
check_exit_status $? "dput .log";

if [ ${mapred_task_partition} -eq 0 ];
then
  hadoop dfs -cp ${mapred_output_dir}/0/lda.par.dump ${mapred_output_dir}/global/topic_counts/lda.par.dump;
